...
| B | **The product pivots**: four sequential acts, each with a reason. (1) pregrade: identify one card, LLM vision as a proof of concept and baseline; (2) live-stream scout: image-to-image identify; (3) bulk scan/lots: identify ported from the scout to remove the LLM and to isolate detect/live-stream failure modes; (4) comps/pricing: raw comps across NM/LP/MP/HP/DMG outgrew "use an API" (D9) | each pivot was a scope/timeline conversation held WITH evidence in hand | `docs/plan/`, git history (3,966 commits since 2026-07-02), memory `trunk-consolidation-*`, `founder-testing-milestones` |
