...
| F | **Agents as labelers** — agents with skills and different vision models label crops; **11–16 s agent wall time per task** (E84; agent-only, excludes human review). The "~3 min by hand → ~10 s" framing is NOT sourced — see D6; the manual baseline was never instrumented. Adversarial pairs, agreement/dissent, $/task | replacing human labeling throughput with an agent pipeline while keeping the human as the verdict of record for hard cases | `docs/experiments/EXP-E28a-*`, `EXP-E84-*` (+ `e28a/`, `e84/` folders), `docs/plan/2026-09-0{6,8,9}-labeling-*`, memory `labeling-program-design-2026-09-09`, `model-words-not-model-ids`, `founder-wont-hunt-ids-on-poor-crops` |
